...
import { getOrderNow, getOrderPre, submitOrder } from '@/services/order'
...
const props = defineProps<{ skuId?: string; count?: string }>()
...
async function getOrderData() {
  if (props.skuId && props.count) {
    const res = await getOrderNow({ skuId: props.skuId, count: props.count })
...
const selectAddress = computed(() => {
  return (
    addressStore.selectAddress || orderPreData.value?.userAddresses.find((item) => item.isDefault)
  )
})
...
<template>
  <scroll-view scroll-y class="viewport">
    ...
    <view class="goods">
      <navigator
        v-for="item of orderPreData?.goods"
        :key="item.skuId"
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    ...
    <view class="button" :class="{ disabled: !selectAddress?.id }" @tap="handleOrderSubmit">
      提交订单
    </view>
  </view>
</template>
