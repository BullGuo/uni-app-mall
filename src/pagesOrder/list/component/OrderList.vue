...
import { payMock } from '@/services/pay'
...
async function getOrderListData() {
  const res = await orderList({ page: 1, pageSize: 5, orderState: props.orderState })
...
async function handlePay(id: string) {
  ...
  const order = orderListData.value.find((item) => item.id === id)
...
<template>
  <scroll-view scroll-y class="orders">
    <view class="card" v-for="item of orderListData" :key="item.id">
      <!-- 订单信息 -->
      <view class="status">
      ...
      <view class="action">
        <!-- 待付款状态：显示去支付按钮 -->
        <template v-if="orderState === OrderState.DaiFuKuan">
        ...
        <template v-else>
          ...
          <!-- 待收货状态: 展示确认收货 -->
          <view v-if="orderState === OrderState.DaiShouHuo" class="button primary">确认收货</view>
        </template>
      </view>
    </view>
    <!-- 底部提示文字 -->
