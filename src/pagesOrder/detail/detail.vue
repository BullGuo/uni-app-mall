...
async function getLogisticsData() {
  const res = await getLogistics(props.id)
...
async function handleReceipt() {
  uni.showModal({
    ...
    success: async (e) => {
      if (e.confirm) {
        const res = await confirmReceipt(props.id)
        orderData.value = res.result
      }
    },
  })
...
function handleOrderDel() {
  uni.showModal({
    title: '是否删除该订单',
    success: async (res) => {
      if (res.confirm) {
        await delOrder([props.id])
...
async function handleOrderCancel() {
  await cancelOrder(props.id, reason.value)
...
<template>
  ...
  >
    <template v-if="orderData">
      ...
      <view class="overview" :style="{ paddingTop: safeAreaInsets!.top + 20 + 'px' }">
        ...
        <template v-else>
          ...
          <view class="button-group">
            ...
            <view
              v-if="orderData.orderState === OrderState.DaiFaHuo"
              class="button"
              @tap="handleConsignment"
            >
              模拟发货
            </view>
            <view
      ...
      <view class="shipment">
        <!-- 订单物流信息 -->
        <view v-for="item of logisticsData" :key="item.id" class="item">
  ...
  <uni-popup ref="popup" type="bottom" background-color="#fff">
    <view class="popup-root">
      ...
      <view class="footer">
        ...
        <view class="button primary" @tap="handleOrderCancel">确认</view>
      </view>
    </view>
  </uni-popup>
</template>
