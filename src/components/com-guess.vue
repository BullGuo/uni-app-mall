...
async function getGuessData() {
  if (isFinish.value) {
    return uni.showToast({ icon: 'none', title: '已经到底了~' })
  }
  const res = await getHomeGuess(pagePar)
  guessData.value.push(...res.result.items)
  if (res.result.pages > pagePar.page) {
    pagePar.page++
  } else {
    isFinish.value = true
  }
...
defineExpose({ getGuessData, resetGuessData })
</script>
